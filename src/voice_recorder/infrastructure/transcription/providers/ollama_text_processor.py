...
class OllamaTextProcessor:
    ...
    def __init__(
        self, config: LocalTranscriptionConfig, console: ConsoleInterface | None = None
    ):
        ...
        try:
            ...
            if response.status_code == 200:
                if console:
                    console.info(
                        f"Ollama text processor initialized with model: {config.ollama_model}"
                    )
            else:
                if console:
                    console.warning(
        ...
        try:
            ...
            response = self.client.post(
                ...
                json={
                    ...
                    "prompt": prompt,
                    "stream": False,
                    "options": {"temperature": self.config.ollama_creativity},
                },
            )
