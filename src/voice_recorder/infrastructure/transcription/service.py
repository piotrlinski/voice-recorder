...
class SimpleTranscriptionService(
    TranscriptionServiceInterface, EnhancedTranscriptionServiceInterface
):
    ...
    def transcribe_and_enhance(self, audio_file_path: str) -> TranscriptionResult:
            ...
            try:
                ...
                if self.console:
                    self.console.warning(
                        f"Text improvement failed: {e}, using original text"
                    )
                return result
        ...
        if self.console and self.text_processor:
            self.console.info(
                "Text processor not available, using original transcription"
            )
    ...
    def enhance_text(self, original_text: str) -> TranscriptionResult:
        ...
        if self.text_processor and self.text_processor.is_available():
            try:
                ...
                if self.console:
                    self.console.warning(
                        f"Text enhancement failed: {e}, using original text"
                    )
                return TranscriptionResult(text=original_text)
